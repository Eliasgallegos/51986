...
import CustomDeclaracionesVisitor from "./CustomDeclaracionesVisitor.js";
...
async function main() {
    ...
    let lexer = new DeclaracionesLexer(inputStream);
    ...
    for (let token of tokens) {
        ...
        const tokenType = DeclaracionesLexer.symbolicNames[token.type] || `UNKNOWN (${token.type})`;
   ...
   lexer = new DeclaracionesLexer(inputStream);
   let tokenStream = new CommonTokenStream(lexer);
   let parser = new DeclaracionesParser (tokenStream);
   let tree = parser.programa();
    ...
    if (parser.syntaxErrorsCount > 0) {
        ...
        const visitor = new CustomDeclaracionesVisitor();
        visitor.visit(tree);
    }
}
function leerCadena() {
